...
/* WINOME: expose color evaluation from a CSS value string so the build-time
 * CSS converter can reuse St's exact color math (st-mix/st-lighten/etc.)
 * instead of reimplementing it. */
gboolean
st_compat_color_from_string (StThemeNode *node,
                             const char  *str,
                             CoglColor   *color)
{
  CRTerm *term;
  ...
  term = cr_term_parse_expression_from_buf ((const guchar *) str);
  if (term == NULL)
    return FALSE;
...
/* Margins are applied by the GTK4 host directly; ClutterActor is unavailable
 * on Windows, so this helper is a no-op in the extraction. */
void
