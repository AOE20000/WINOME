...
/* Rendering state is unused in the Windows st-compat extraction (GTK4 does
 * the painting); keep an empty struct so the value type still exists. */
...
/* WINOME: evaluate a CSS color value string (may contain st-mix()/st-lighten()
 * /st-darken()/st-transparentize()/-st-accent-color) using St's exact math.
 * Returns FALSE if the string is not a valid color expression. */
gboolean st_compat_color_from_string (StThemeNode *node,
                                      const char  *str,
                                      CoglColor   *color);
